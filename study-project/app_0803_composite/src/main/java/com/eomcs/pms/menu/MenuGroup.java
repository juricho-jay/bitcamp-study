...
public class MenuGroup extends Menu {
  ...
  // 컴파일 오류가 발생한다. 
  // Menu 클래스는 기본 생성자가 없다.
  // 따라서 개발자가 직접 생성자를 정의해야 한다.
  public MenuGroup(String title) {
    super(title);
  }

  public MenuGroup(String title, boolean disablePrevMenu) { //개발자에게 선택권을 주려고 할 때 생성자 추가로 만들음
    super(title);
  ...
  public void execute() {
    while (true) {
      System.out.printf("\n[%s]\n", this.title);
      for (int i = 0; i < this.size; i++) {
        System.out.printf("%d. %s\n", i + 1, this.childs[i].title); // 1번부터 출력하려고 i + 1
      }
      ...
      this.childs[menuNo - 1].execute(); // i + 1해서 menuNo - 1 해야함
    }
